...
const C = {
  ...
  brand: "oklch(0.45 0.07 300)", brandDark: "oklch(0.38 0.06 300)", terracotta: "oklch(0.53 0.12 40)",
  success: "oklch(0.50 0.04 150)", successTint: "oklch(0.94 0.02 150)",
};

// Plum-hue tints used for AI / delivery / brand blocks.
const BRAND_TINT = "oklch(0.93 0.025 300)";
...
function PurchaseGate({ productId, productName, onVerified }) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const data = await r.json();
      if (data.verified) {
        try { localStorage.setItem("rc-access-" + productId, JSON.stringify({ verified: true, product: productId })); } catch {}
        try { if (window.storage?.set) window.storage.set("rc-access-" + productId, JSON.stringify({ verified: true, product: productId })); } catch {}
        onVerified();
      } else {
        setError(data.message || "No matching purchase found for this email.");
      }
    } catch {
      setError("preview");
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: UI, color: C.text, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", maxWidth: 480, margin: "0 auto", padding: "clamp(28px, 9vw, 64px) 24px 48px" }}>
        ...
        {error === "preview" ? (
          <div className="rc-rise" style={{ marginTop: 20, textAlign: "center" }}>
            <p style={{ fontSize: 13, color: C.muted, marginBottom: 12, fontFamily: UI }}>Purchase verification requires Netlify deployment.</p>
            <button onClick={onVerified} style={{ padding: "12px 22px", borderRadius: 12, border: `1px solid ${C.line}`, background: C.surface, color: C.text, fontSize: 13, fontWeight: 600, fontFamily: UI, cursor: "pointer" }}>Enter preview mode</button>
          </div>
        ) : error ? (
          <div className="rc-rise" style={{ marginTop: 20, background: "oklch(0.945 0.035 42)", borderRadius: 12, padding: "14px 16px" }}>
            <p style={{ fontSize: 13, color: C.terracotta, lineHeight: 1.55, margin: 0, fontFamily: UI }}>{error}</p>
          </div>
        ) : null}
    ...
    (async () => {
      try {
        if (window.storage?.get) {
          ...
          if (r?.value) { const d = JSON.parse(r.value); if (d.verified) { setScreen("app"); return; } }
        }
      } catch {}
      try {
        const s = localStorage.getItem("rc-access-research");
        if (s) { const d = JSON.parse(s); if (d.verified) setScreen("app"); }
      } catch {}
    })();
  }, []);
