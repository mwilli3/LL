...
const RECS = {
  ...
  hydration_cortisol: { product:"Pure Magnesium Power", desc:"Your hydration foundation shows a pattern worth noting. Even mild dehydration can elevate cortisol, and magnesium is involved in over 300 enzymatic processes including hydration regulation at the cellular level. Magnesium may support the connection between your hydration foundation and your nervous system.", price:"$26.99", url:"https://bdyalign.com/products/pure-magnesium-power" },
...
function Accordion({ title, children }) {
  ...
  return (
    <div style={{borderTop:`1px solid ${B.tx}12`,marginBottom:8}}>
      ...
      {open && <div style={{paddingBottom:20,animation:"up .3s cubic-bezier(0.23,1,0.32,1) both"}}>{children}</div>}
    </div>
  );
}
...
Keep it under 200 words. Warm but direct. No bullet points \u2014 flowing paragraphs.\n\nFinally, on a NEW line at the very end, write exactly one of these tags based on the PRIMARY pattern you identified (parsed programmatically, write ONLY the tag):\n[CONSISTENCY_GAP] [ENERGY_DEPLETION] [HYDRATION_CORTISOL] [FOUNDATION_BUILDING]`}]
  ...
  const verifyPurchase = async () => {
    ...
    const PRODUCT_URL = "https://lovelarice.com/products/rooted-reset-challenge-kit";
    try {
      ...
      const d = await res.json();
      if (d && d.verified) {
        localStorage.setItem("larice_rooted_kit_email", gateEmail.trim());
