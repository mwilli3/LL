...
const B = { pri:"#A84A30", sec:"#D4856A", acc:"#5C5470", bg:"#FAF6F2", tx:"#3A2018", txm:"#6B5B52", txl:"#A69890", wh:"#FFFFFF", fill:"#F5E8E1", rose:"#8B3A4A", roseL:"#F5ECF0", roseD:"#6B2A38" };
...
export default function App() {
  ...
  const Lbl = ({children}) => (
    <div style={{display:"flex",alignItems:"baseline",gap:12,marginBottom:18}}>
      <span style={{height:1,width:26,background:B.rose,opacity:.45,transform:"translateY(-5px)"}} />
      <span style={{fontFamily:H,fontSize:20,fontWeight:600,fontStyle:"italic",color:B.rose,letterSpacing:.2}}>{children}</span>
    </div>
  );
          ...
          <div style={{animation:"up .4s cubic-bezier(.22,1,.36,1) both"}}>
            ...
            {QUESTIONS.map((q,i)=>(
              <div key={i} style={{marginBottom:20}}>
                <p style={{fontSize:13,fontWeight:600,color:B.rose,marginBottom:8}}>{q.q}</p>
                <div style={{padding:"16px 18px",borderRadius:10,background:B.wh,border:`1px solid ${B.rose}12`,minHeight:40}}>
  ...
  return (
    <div style={css}>
      ...
      <div ref={ref} style={{...wrap,paddingTop:40,paddingBottom:56}}>
        ...
        {checked && (
          <div style={{background:B.roseL,borderRadius:10,padding:"16px 20px",marginBottom:24,display:"flex",alignItems:"center",gap:14,border:`1px solid ${B.rose}20`}}>
            <div style={{width:8,height:8,borderRadius:"50%",background:B.rose,minWidth:8}} />
            <p style={{fontSize:14,color:B.rose,fontWeight:500}}>Boundaries checked tonight</p>
          </div>
        )}
        ...
        <div style={{textAlign:"center",marginBottom:40,animation:"up .6s cubic-bezier(.22,1,.36,1) .15s both"}}>
          <h1 style={{fontFamily:H,fontSize:"clamp(40px,13vw,58px)",fontWeight:600,lineHeight:.96,letterSpacing:-1,marginBottom:18}}>
            Three questions.<br/><span style={{fontStyle:"italic",color:B.rose}}>Five minutes.</span>
          </h1>
          ...
          {QUESTIONS.map((q,i)=>(
            <div key={i} style={{marginBottom:20}}>
              <div style={{display:"flex",alignItems:"baseline",gap:12,marginBottom:10}}>
                <span style={{fontFamily:H,fontSize:24,fontWeight:600,fontStyle:"italic",color:B.rose,lineHeight:.9}}>{`0${i+1}`}</span>
              ...
              <textarea value={todayEntry[q.key]||""} onChange={e=>updateToday(q.key,e.target.value)} placeholder={q.placeholder}
                style={{width:"100%",minHeight:70,padding:14,fontSize:14,fontFamily:F,border:`1px solid ${B.rose}15`,borderRadius:10,background:B.wh,color:B.tx,outline:"none",boxSizing:"border-box",resize:"vertical",lineHeight:1.7}}
                onFocus={e=>e.target.style.borderColor=B.rose} onBlur={e=>e.target.style.borderColor=B.rose+"15"} />
            </div>
          ))}
        </div>
        ...
        {patterns.total > 0 && (
          <div style={{animation:"up .5s cubic-bezier(.22,1,.36,1) .25s both",marginBottom:32}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <div style={{flex:1,padding:"18px 16px",borderRadius:10,background:B.wh,border:`1px solid ${B.rose}12`,textAlign:"center"}}>
                <p style={{fontSize:28,fontWeight:600,color:B.rose,lineHeight:1}}>{patterns.total}</p>
                <p style={{fontSize:11,color:B.txm,marginTop:4,fontWeight:500}}>entries</p>
              </div>
              <div style={{flex:1,padding:"18px 16px",borderRadius:10,background:B.wh,border:`1px solid ${B.rose}12`,textAlign:"center"}}>
                <p style={{fontSize:28,fontWeight:600,color:B.rose,lineHeight:1}}>{patterns.withAll}</p>
                <p style={{fontSize:11,color:B.txm,marginTop:4,fontWeight:500}}>complete</p>
              </div>
              <div style={{flex:1,padding:"18px 16px",borderRadius:10,background:B.wh,border:`1px solid ${B.rose}12`,textAlign:"center"}}>
                <p style={{fontSize:28,fontWeight:600,color:B.rose,lineHeight:1}}>{patterns.noCount}</p>
                <p style={{fontSize:11,color:B.txm,marginTop:4,fontWeight:500}}>boundaries set</p>
              </div>
            </div>
          </div>
        )}
            ...
            {pastEntries.slice(0,7).map(([date,entry],i)=>{
              ...
              return (
                <button key={i} onClick={()=>{setHistoryDate(date);setView("detail");}}
                  style={{display:"flex",width:"100%",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",marginBottom:6,background:B.wh,border:`1px solid ${B.rose}10`,borderRadius:10,cursor:"pointer",fontFamily:F,textAlign:"left",transition:"all .2s"}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor=B.rose;e.currentTarget.style.background=B.roseL+"70"}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor=B.rose+"10";e.currentTarget.style.background=B.wh}}>
                  ...
                  <div style={{display:"flex",gap:4,marginLeft:12}}>
                    {["no","protect","claim"].map((k,j)=>(
                      <div key={j} style={{width:6,height:6,borderRadius:"50%",background:entry[k]&&entry[k].trim()?B.rose:B.txl+"40"}} />
                    ))}
                  </div>
                </button>
              );
        ...
        {/* Archetype context */}
        <div style={{animation:"up .5s cubic-bezier(.22,1,.36,1) .35s both",padding:"22px 20px",borderRadius:12,background:B.roseL,border:`1px solid ${B.rose}18`,marginBottom:32}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:20,margin:"8px 0 28px",padding:"28px 0",borderTop:`1px solid ${B.tx}12`,borderBottom:`1px solid ${B.tx}12`}}>
          <p style={{fontFamily:H,fontSize:"clamp(72px,24vw,108px)",fontWeight:600,color:B.rose,lineHeight:.8,letterSpacing:-3}}>{streak}</p>
          <div>
