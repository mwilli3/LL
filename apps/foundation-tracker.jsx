...
const B = { pri:"#A84A30", sec:"#D4856A", acc:"#5C5470", bg:"#FAF6F2", tx:"#3A2018", txm:"#6B5B52", txl:"#A69890", wh:"#FFFFFF", fill:"#F5E8E1", grn:"#5A7F3C", grnL:"#ECF2E6", grnD:"#3D5A28" };
...
export default function App() {
  ...
  const Lbl = ({children}) => (
    <div style={{display:"flex",alignItems:"baseline",gap:12,marginBottom:18}}>
      <span style={{height:1,width:26,background:B.grn,opacity:.45,transform:"translateY(-5px)"}} />
      <span style={{fontFamily:H,fontSize:20,fontWeight:600,fontStyle:"italic",color:B.grn,letterSpacing:.2}}>{children}</span>
    </div>
  );
  ...
  return (
    <div style={css}>
      ...
      <div ref={ref} style={{...wrap,paddingTop:40,paddingBottom:56}}>
        ...
        {checked && (
          <div style={{background:B.grnL,borderRadius:10,padding:"16px 20px",marginBottom:24,display:"flex",alignItems:"center",gap:14,border:`1px solid ${B.grn}20`}}>
            <div style={{width:8,height:8,borderRadius:"50%",background:B.grn,minWidth:8}} />
            <p style={{fontSize:14,color:B.grn,fontWeight:500}}>Checked in today</p>
          </div>
        )}
        ...
        <div style={{textAlign:"center",marginBottom:40,animation:"up .6s cubic-bezier(.22,1,.36,1) .15s both"}}>
          {dayNum > 0 && (
            <div style={{display:"flex",justifyContent:"center",alignItems:"baseline",gap:8,marginBottom:14}}>
              <span style={{fontFamily:H,fontSize:30,fontWeight:600,fontStyle:"italic",color:B.grn,lineHeight:1}}>Day {dayNum}</span>
          ...
          <h1 style={{fontFamily:H,fontSize:"clamp(40px,13vw,58px)",fontWeight:600,lineHeight:.96,letterSpacing:-1,marginBottom:18}}>
            Four foundations.<br/><span style={{fontStyle:"italic",color:B.grn}}>Seven days.</span>
          </h1>
          ...
          <div style={{display:"flex",gap:10}}>
            <div style={{flex:1}}>
              ...
              <input value={data.hydrationTarget||""} onChange={e=>setData({...data,hydrationTarget:e.target.value})} placeholder="e.g. 80 oz"
                ...
                onFocus={e=>e.target.style.borderBottomColor=B.grn} onBlur={e=>e.target.style.borderBottomColor=B.txl+"50"} />
            </div>
            <div style={{flex:1}}>
        ...
        <div style={{animation:"up .5s cubic-bezier(.22,1,.36,1) .25s both",marginBottom:28}}>
          ...
          <div style={{background:B.wh,borderRadius:12,padding:"20px",border:`1px solid ${B.grn}15`}}>
            {FOUNDS.map((f,i)=>{
              ...
              return (
                <div key={i} style={{display:"flex",alignItems:"center",gap:14,marginBottom:i<3?16:0,paddingBottom:i<3?16:0,borderBottom:i<3?`1px solid ${B.tx}08`:"none"}}>
                  <button onClick={()=>toggle(f.key)} style={{width:28,height:28,minWidth:28,borderRadius:6,border:`2px solid ${isDone?B.grn:B.txl+"60"}`,background:isDone?B.grn:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .2s"}}>
        ...
        <div style={{animation:"up .5s cubic-bezier(.22,1,.36,1) .3s both",marginBottom:28}}>
          ...
          <textarea value={todayData.notice||""} onChange={e=>{
            ...
            style={{width:"100%",minHeight:80,padding:14,fontSize:14,fontFamily:F,border:`1px solid ${B.grn}15`,borderRadius:10,background:B.wh,color:B.tx,outline:"none",boxSizing:"border-box",resize:"vertical",lineHeight:1.7}}
            onFocus={e=>e.target.style.borderColor=B.grn} onBlur={e=>e.target.style.borderColor=B.grn+"15"} />
        </div>
                ...
                {dayDates.map((date,i)=>{
                  ...
                  return (
                    <div key={i} style={{flex:1,textAlign:"center"}}>
                      <p style={{fontSize:10,color:isToday?B.grn:B.txm,fontWeight:isToday?600:400,marginBottom:6}}>{dayLabel(date)}</p>
                      <div style={{display:"flex",flexDirection:"column",gap:2}}>
                        {[0,1,2,3].map(j=>(
                          <div key={j} style={{height:8,borderRadius:2,background:count>j?B.grn:(isPast||isToday)?`${B.tx}0c`:`${B.tx}06`,transition:"background .3s"}} />
                        ))}
                      </div>
                      <p style={{fontSize:10,color:count>0?B.grn:B.txl,fontWeight:500,marginTop:4}}>{count}/4</p>
                    </div>
                  );
                })}
              </div>
              <div style={{display:"flex",gap:10,marginTop:14,justifyContent:"center",borderTop:`1px solid ${B.tx}08`,paddingTop:10}}>
                {FOUNDS.map((f,i)=>{
                  ...
                  return <div key={i} style={{textAlign:"center",flex:1}}>
                    <p style={{fontSize:18,fontWeight:600,color:B.grn,lineHeight:1}}>{total}</p>
        ...
        {dayNum >= 7 && (
          <div style={{animation:"up .5s cubic-bezier(.22,1,.36,1) .4s both",marginBottom:32}}>
            ...
            <div style={{padding:"22px 20px",borderRadius:10,background:B.grnL,border:`1px solid ${B.grn}18`}}>
              ...
              <textarea value={data.reflection||""} onChange={e=>setData({...data,reflection:e.target.value})} placeholder="Write here..."
                style={{width:"100%",minHeight:100,padding:14,fontSize:14,fontFamily:F,border:`1px solid ${B.grn}20`,borderRadius:8,background:B.wh,color:B.tx,outline:"none",boxSizing:"border-box",resize:"vertical",lineHeight:1.7}}
                onFocus={e=>e.target.style.borderColor=B.grn} onBlur={e=>e.target.style.borderColor=B.grn+"20"} />
            </div>
          </div>
        )}

        {/* The rule */}
        <div style={{animation:"up .5s cubic-bezier(.22,1,.36,1) .4s both",padding:"22px 20px",borderRadius:12,background:B.grnL,border:`1px solid ${B.grn}18`,marginBottom:32}}>
          <Lbl>The rule</Lbl>
        ...
        <div style={{animation:"up .5s cubic-bezier(.22,1,.36,1) .45s both",marginBottom:32}}>
          ...
          {[
            ...
            <div key={i} style={{display:"flex",gap:18,alignItems:"baseline",padding:"20px 0",borderBottom:i<3?`1px solid ${B.tx}12`:"none"}}>
              <span style={{fontFamily:H,fontSize:30,fontWeight:600,fontStyle:"italic",color:B.grn,lineHeight:.9,minWidth:38}}>{`0${i+1}`}</span>
              <div>
        ...
        {/* Upgrade CTA */}
        <div style={{marginTop:8,padding:"28px 24px",background:B.grn,borderRadius:12,textAlign:"center",marginBottom:32}}>
          <p style={{fontSize:10,fontWeight:600,letterSpacing:3,textTransform:"uppercase",color:"rgba(255,255,255,.5)",marginBottom:8}}>Ready for 30 days?</p>
          <p style={{fontSize:18,fontWeight:600,color:B.wh,marginBottom:8}}>Rooted Reset Challenge Kit</p>
          <p style={{fontSize:13,lineHeight:1.7,color:"rgba(255,255,255,.75)",marginBottom:20,maxWidth:320,margin:"0 auto 20px"}}>30-day tracker, miss protocol, identity prompts, weekly audits, and AI-powered foundation analysis.</p>
          <button style={{padding:"14px 36px",fontSize:14,fontWeight:600,fontFamily:F,color:B.grn,background:B.wh,border:"none",borderRadius:8,cursor:"pointer",letterSpacing:.5,textTransform:"uppercase",transition:"all .2s"}}
        ...
        <div style={{display:"flex",alignItems:"center",gap:20,margin:"8px 0 28px",padding:"28px 0",borderTop:`1px solid ${B.tx}12`,borderBottom:`1px solid ${B.tx}12`}}>
          <p style={{fontFamily:H,fontSize:"clamp(72px,24vw,108px)",fontWeight:600,color:B.grn,lineHeight:.8,letterSpacing:-3}}>{streak}</p>
          <div>
