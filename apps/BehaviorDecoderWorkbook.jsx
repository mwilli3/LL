...
const C = {
  ...
  // plum brand tints
  brandTint: "oklch(0.93 0.025 300)",
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: UI, color: C.text, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", maxWidth: 520, margin: "0 auto", padding: "clamp(28px, 9vw, 64px) 24px 48px" }}>
        ...
        {error === "preview" ? (
          <div className="rc-rise" style={{ marginTop: 20 }}>
            <p style={{ fontSize: 13, color: C.muted, marginBottom: 12, lineHeight: 1.55 }}>Purchase verification requires Netlify deployment.</p>
            <button onClick={onVerified} style={{ padding: "12px 22px", borderRadius: 12, border: `1px solid ${C.line}`, background: C.surface, color: C.muted, fontSize: 13, fontWeight: 600, fontFamily: UI, cursor: "pointer" }}>Enter preview mode</button>
          </div>
        ) : error ? (
          <div className="rc-rise" style={{ marginTop: 20, background: C.fightBg, borderRadius: 12, padding: "14px 16px" }}>
            <p style={{ fontSize: 13, color: C.terracotta, lineHeight: 1.55, margin: 0 }}>{error}</p>
          </div>
        ) : null}
